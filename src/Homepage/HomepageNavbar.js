...
const HomepageNavbar = () => {
  ...
  // let visibleNames = Array.isArray(mainmenu) ? [...mainmenu] : [];

  // if (windowWidth <= 768) {
  //   visibleNames = mainmenu.slice(0, 4);
  // } else {
  //   const maxVisibleNames = Math.floor(windowWidth / 160); // Adjust 150 to your desired name width
  //   visibleNames = visibleNames.slice(0, maxVisibleNames);
  // }
  let visibleNames = Array.isArray(mainmenu) ? [...mainmenu] : [];

if (windowWidth <= 768) {
  visibleNames = Array.isArray(mainmenu) ? mainmenu.slice(0, 4) : [];
} else {
  const maxVisibleNames = Math.floor(windowWidth / 160);
  visibleNames = Array.isArray(visibleNames) ? visibleNames.slice(0, maxVisibleNames) : [];
}
