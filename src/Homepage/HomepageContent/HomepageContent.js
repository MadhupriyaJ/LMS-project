...
const HomepageContent = () => {
  ...
  const [customersubcourse, setCustomerSubcourse] = useState([]);
  ...
  const fetchsubcourse = async () => {
    try {
      const response = await fetch("/customerSubcourse");
      const data = await response.json();
      setCustomerSubcourse(data);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  <section className="category-best-selling noprice">
    <div className="category-best-sellingcontainer">
      <div className="displaycards">
        {Array.isArray(customersubcourse) ? (
          customersubcourse.map((course) => {
            if (pathname.slice(1) === course.category) {
              console.log(
                "pathname.slice(1) === course.category",
                pathname.slice(1) === course.category
      ...
      <div className="w-10/12 mx-auto">
        ...
        {Array.isArray(mastercourse) && mastercourse.map((course) => {
          if (pathname.slice(1) === course.category) {
            console.log(
              "pathname.slice(1) === course.category",
              pathname.slice(1) === course.category
